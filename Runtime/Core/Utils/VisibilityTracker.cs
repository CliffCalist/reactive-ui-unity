...
{
    [Serializable]
    public class VisibilityTracker : IDisposable
    {
        ...
        public void Init()
        {
            ...
            IsAnyShowed = Observable.CombineLatest(_views.Select(v => v.Visibility.IsSelfShowed))
                .Select(v => v.Any(v => v))
                .ToReadOnlyReactiveProperty()
                .AddTo(ref disposableBuilder);

            IsAnyHided = Observable.CombineLatest(_views.Select(v => v.Visibility.IsSelfShowed))
                .Select(v => v.Any(v => !v))
                .ToReadOnlyReactiveProperty()
                .AddTo(ref disposableBuilder);

            IsAllShowed = Observable.CombineLatest(_views.Select(v => v.Visibility.IsSelfShowed))
                .Select(v => v.All(v => v))
                .ToReadOnlyReactiveProperty()
                .AddTo(ref disposableBuilder);

            IsAllHided = Observable.CombineLatest(_views.Select(v => v.Visibility.IsSelfShowed))
                .Select(v => v.All(v => !v))
                .ToReadOnlyReactiveProperty()
                .AddTo(ref disposableBuilder);
